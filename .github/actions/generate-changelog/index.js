...
Object.defineProperty(exports, "__esModule", ({ value: true }));
__export(__nccwpck_require__(5016));
//# sourceMappingURL=index.js.map
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const fs_1 = __nccwpck_require__(7147);
const debug_1 = __importDefault(__nccwpck_require__(2135));
...
var init_util = __esm({
  "src/lib/utils/util.ts"() {
    import_file_exists = __nccwpck_require__(9929);
    NULL = "\0";
    NOOP = () => {
    };
...
var init_completion_detection_plugin = __esm({
  "src/lib/plugins/completion-detection.plugin.ts"() {
    import_promise_deferred = __nccwpck_require__(123);
    init_utils();
...
var init_git_logger = __esm({
  "src/lib/git-logger.ts"() {
    import_debug = __toESM(__nccwpck_require__(2135));
    init_utils();
...
var init_scheduler = __esm({
  "src/lib/runners/scheduler.ts"() {
    init_utils();
    import_promise_deferred2 = __nccwpck_require__(123);
    init_git_logger();
    createScheduledTask = (() => {
      let id = 0;
      return () => {
...
console.log("Running");
const core_1 = __nccwpck_require__(7954);
const src_1 = __nccwpck_require__(8891);
/**
 * Converts a comma separated list of strings into a `string[]`.
 */
function getOptionalStringArrayInput(name) {
    var _a;
    return (_a = getOptionalInput(name)) === null || _a === void 0 ? void 0 : _a.split(",").map((scope) => scope.trim());
}
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const outputs = yield (0, src_1.generateChangelog)(options);
        Object.entries(outputs).map(([key, value]) => (0, core_1.setOutput)(key, value));
    });
}
main().catch((err) => {
    console.error(err);
...
const simple_git_1 = __importDefault(__nccwpck_require__(7153));
const options_1 = __nccwpck_require__(9961);
const git_1 = __nccwpck_require__(7364);
const versioning_1 = __nccwpck_require__(1756);
...
function generateChangelog(options = {}, git = (0, simple_git_1.default)()) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const nextVersion = (0, versioning_1.getNextVersion)({ prevVersion, changes });
        const nextTag = (0, versioning_1.getTagPrefix)({ options }) + nextVersion;
...
function renderChangelog({ changes, options, }) {
    var _a, _b, _c;
    const lines = [];
    if (options.prefix)
        lines.push(options.prefix.trim());
    if (changes.features.length > 0) {
        const featuresText = (_a = options.fixHeading) !== null && _a !== void 0 ? _a : options_1.DEFAULT_FEAT_HEADING;
        lines.push("", `### ${featuresText}`, "", ...changes.features.map(formatCommit(options)));
    }
    if (changes.fixes.length > 0) {
        const fixesText = (_b = options.fixHeading) !== null && _b !== void 0 ? _b : options_1.DEFAULT_FIX_HEADING;
        lines.push("", `### ${fixesText}`, "", ...changes.fixes.map(formatCommit(options)));
    }
    if (changes.breakingChanges.length > 0) {
        const breakingChangesText = (_c = options.breakingChangeHeading) !== null && _c !== void 0 ? _c : options_1.DEFAULT_BREAKING_CHANGE_HEADING;
        lines.push("", `### ${breakingChangesText}`, "", ...changes.breakingChanges.map(formatCommit(options)));
    }
    if (options.suffix)
        lines.push(options.suffix.trim());
    return lines.join("\n").trim();
}
function formatCommit({ changeTemplate, }) {
    const template = (0, templater_js_1.default)(changeTemplate !== null && changeTemplate !== void 0 ? changeTemplate : options_1.DEFAULT_CHANGE_TEMPLATE);
    return (commit) => {
        let scope;
        let message;
        const scopeRegex = /.*\((.*?)\):\s*?(.*)/;
...
function getBreakingChanges(commit) {
    const changes = [];
    if (commit.message.includes("!:"))
        changes.push(commit);
    commit.body.split("\n").forEach((line) => {
        if (line.startsWith("BREAKING CHANGE:"))
            changes.push(Object.assign(Object.assign({}, commit), { message: line }));
    });
    return changes;
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.DEFAULT_CHANGE_TEMPLATE = exports.DEFAULT_FIX_HEADING = exports.DEFAULT_FEAT_HEADING = exports.DEFAULT_BREAKING_CHANGE_HEADING = void 0;
exports.DEFAULT_BREAKING_CHANGE_HEADING = "BREAKING CHANGES";
exports.DEFAULT_FEAT_HEADING = "Features";
exports.DEFAULT_FIX_HEADING = "Bug Fixes";
exports.DEFAULT_CHANGE_TEMPLATE = "- {{ #if (scope) }}**{{ scope }}:** {{ /if }}{{ message }} ({{ commit.hash }})";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.findRelevantCommits = exports.findPrevTag = void 0;
const versioning_1 = __nccwpck_require__(1756);
...
exports.findPrevTag = findPrevTag;
function findRelevantCommits({ git, prevTag, options, }) {
    return __awaiter(this, void 0, void 0, function* () {
        let commits;
        if (!prevTag) {
...
exports.findRelevantCommits = findRelevantCommits;
function filterCommits(options) {
    var _a;
    let regex;
    if (options.module) {
        const scopes = (_a = options.scopes) !== null && _a !== void 0 ? _a : [(0, versioning_1.getLowercaseModule)(options.module)];
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getNextVersion = exports.getPrevVersion = exports.getTagPrefix = exports.getLowercaseModule = void 0;
function getLowercaseModule(module) {
    return module.toLowerCase().replace(/\s/gm, "-");
}
exports.getLowercaseModule = getLowercaseModule;
function getTagPrefix({ options }) {
    if (options.module)
        return `${getLowercaseModule(options.module)}-v`;
    return "v";
}
exports.getTagPrefix = getTagPrefix;
function getPrevVersion({ prevTag, options, }) {
    return prevTag === null || prevTag === void 0 ? void 0 : prevTag.replace(getTagPrefix({ options }), "");
}
